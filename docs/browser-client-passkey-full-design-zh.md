...
## 2.1 核心原理
- 通行密钥是 WebAuthn/FIDO2 公私钥认证，不是密码字符串。
- 注册时生成一对密钥：
...
| 数据 | 存放位置 | 是否可同步到你自己的云 |
|---|---|---|
| `privateKey`（通行密钥私钥） | 认证器（系统或你的托管保险库） | 可以（仅密文，E2EE） |
| `publicKey`、`credentialId` | 网站服务器（RP） | 不归你同步 |
| 通行密钥元数据（站点、用户名、创建时间、设备） | 你的本地数据库 | 可以 |
...
- 所有 passkey 操作强校验：
  ...
  - `rpId`
  - `top-level frame` 场景
- 禁止在 http 非安全上下文执行通行密钥流程（开发本地例外白名单）。
