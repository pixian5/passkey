...
### 阶段二：Docker、同步和生产部署（代码已完成，远程发布待执行）
...
当前代码已通过本机 Rust/API 测试、Docker Compose 配置校验和 Web `/healthz` 烟测；远程 Ubuntu 主机部署、多架构镜像发布和生产回滚尚未在本次本机修改中执行。
...
- 1.1.1 起：vault 落盘带 `fsync`；本地命令失败回滚内存；同步网络 I/O 不长期占用全局 vault 锁；Bearer 常量时间比较；主密码不 `trim`。
