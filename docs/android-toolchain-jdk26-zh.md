...
Gradle 官方兼容表要求使用 JDK 26 运行 Gradle 时至少使用 Gradle 9.4.0，因此项目采用 9.7.0。AGP 9.3.1 要求 Gradle 至少 9.5.0，并且已经内置 Kotlin；模块不再应用 `org.jetbrains.kotlin.android` 插件。
...
错误中的 `25.0.3` 或 `26.0.2` 若出现在 `JavaVersion.parse` 堆栈中，表示 Gradle 启动时使用的 JDK 版本，不是 Android SDK Build Tools 版本。`build-tools;26.0.2` 可以单独安装，但不能修复该 Java 版本解析错误。
