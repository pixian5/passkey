...
| 从回收站恢复账号 | 插入全部列表顶部 | 插入所有仍有效的原文件夹顶部 |
...
软删除账号会保留其 `folderIds`，因此恢复时可以回到之前仍存在的所有文件夹，并在这些文件夹中置顶。账号恢复后不会回到删除前的旧位置，而是按“新加入账号”规则插入顶部。文件夹永久删除后，随文件夹删除其 `regularAccountIds` 字段；已删除或永久删除的文件夹不会因恢复账号重新出现。
