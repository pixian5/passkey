...
- Tauri 的补偿任务按目标 URL 规范化（域名大小写、默认端口、尾部斜杠），自动同步遵守 5 秒起步、最高 1280 秒的退避；用户点击“立即重试补偿任务”可跳过等待，但继续复用同一次逻辑写入的幂等/会话/操作 ID。同步页显示失败次数、下次重试时间和最后错误，也可清理已禁用目标的任务。
- Docker Web 当前没有独立持久化补偿队列；共享 UI 对应查询返回空列表、清理返回 0。它仍执行正常同步命令和服务端条件写入，不能把这一命令兼容层描述成已经拥有 outbox。
...
版本 `1.3.7` 的同步复核基线：扩展测试 97 项、Core `pass-merge` 30 项、Tauri 27 项、同步服务器 Python 测试 36 项及端到端测试 4 项，Web 12 项，Swift `swift build`、严格 Clippy、命令矩阵与 JS/Rust 对拍均通过。同步报告统一保留兼容字段并增加报告版本、安全结果、阶段、来源、重试状态、会话/操作 ID、错误码和 revision；UI 可区分“已写本地”“已推送”“待补偿”。Chrome、Swift 和 Tauri 的补偿队列会保存 payload 摘要、ETag/revision 和完整幂等上下文；同一目标且摘要相同才是同一次逻辑写入，重启后复用 key，摘要改变则重置重试次数并创建新 key。Tauri 的 `sync_outbox.json` 使用本地 vault 加密保存，覆盖自建服务器和 WebDAV，并按规范化目标执行真实退避、自动定时重试和手动强制重试；安全检查阻止的预览不会写入队列，成功推送后自动清除。Docker Web 没有独立持久化 outbox。自建服务器审计表记录客户端会话/操作/设备/版本；它们不是凭据，不写入 payload 或密码。账号实体数组按稳定 `recordId`（再按 `accountId`）作传输规范化，用户可见顺序仍只由顶层和文件夹顺序数组决定。扩展用单次激活认领避免一次点击密码框产生两次账号选择或两次提示。Docker 真实镜像结果以本轮实际完成情况为准；测试数量只描述该版本实际运行结果，测试增删后必须重新更新。
