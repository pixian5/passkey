...
36. Google 注册失败的诊断必须区分浏览器返回阶段与 RP 服务端确认阶段。Google 的 `FinishPasskeyEnrollment` 只消费 `clientDataJSON`、`attestationObject`、transports、UV 结果和 authenticator attachment；当 `1.5.5` 报告显示匿名证明组合一致但 Google 仍拒绝时，根因是 RP 要求可验证直接证明，后续版本应观察 Chrome 原生认证器路径，而不是继续修改 Google 未提交的 `rawId` 或客户端扩展输出。Pass 诊断报告记录 `aaguidIsZero`、`anonymousAttestation` 和 `anonymousAttestationIsConsistent`，便于确认匿名证明组合是否一致。
