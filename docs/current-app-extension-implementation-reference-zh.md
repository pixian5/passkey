...
> 文档性质：**当前代码事实**，不是目标蓝图。版本以仓库根目录 `VERSION` 为唯一来源，当前为 `1.1.8`。
>
...
版本 `1.1.8` 的已验证基线：版本落点 45 项、UI 命令 68 个、扩展测试 79 项、Docker Web 9 项、Tauri 22 项、同步服务器 33 项、脚本测试 23 项。Core 门禁、Clippy correctness、Docker Compose 解析、JSON Schema 文件解析、Shell 语法、Markdown 本地链接和 Swift/Xcode 构建也纳入本轮验证。数字只描述该版本测试发现量，测试增删后应重新运行并更新，不能永久照抄。
