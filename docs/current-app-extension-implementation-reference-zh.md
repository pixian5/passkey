...
> 当前为 `1.6.7`；完成本轮全项目文档同步后必须由版本脚本递增并校验。
>
...
Tauri/Swift 的 SSH 创建服务不再使用 `accept-new`。首次连接会用 `ssh-keyscan` 读取并自动写入应用私有 `known_hosts`；22 端口使用主机名，非 22 端口必须精确匹配 `[主机]:端口`。之后统一使用 `StrictHostKeyChecking=yes`，主机密钥变化会被拒绝。
