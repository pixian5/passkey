...
> 当前为 `1.7.3`；完成本轮全项目文档同步后必须由版本脚本递增并校验。
>
...
Tauri/Swift 的 SSH 创建服务不再使用 `accept-new`。首次连接会用 `ssh-keyscan` 读取并自动写入应用私有 `known_hosts`；22 端口使用主机名，非 22 端口必须精确匹配 `[主机]:端口`。之后统一使用 `StrictHostKeyChecking=yes`，主机密钥变化会被拒绝。首次自动写入属于无提示 TOFU，不能抵御首次连接时的 DNS 污染或中间人；项目明确接受这一交互取舍，使用者应优先采用私钥认证并确保首次连接网络可信。
...
30. Pass 新建的托管通行密钥是同步型、多设备凭据：注册认证数据使用 `UP|UV|BE|BS|AT`（`0x5d`），断言使用 `UP|UV|BE|BS`（`0x1d`）。注册证明使用匿名 `fmt: "none"` 与全零 AAGUID；`backupEligible`、`backupState` 随凭据同步并在合并时保守保留，创建这些字段前的旧凭据继续使用原有设备绑定标志。
...
32. `attestation: "direct"` 或 `"enterprise"` 是 RP 的证明偏好。Pass 没有受信任的认证器证明证书链和 FIDO Metadata 条目：明确要求 `direct`/`enterprise` 的注册交给 Chrome/系统原生认证器；普通注册由 Pass 返回匿名 `fmt: "none"`、空 `attStmt` 与全零 AAGUID。不得伪造厂商身份或冒用其他产品的 AAGUID。
...
历史复核记录：版本 `1.4.4` 曾完成扩展共享层、Core Rust、JS/Rust 合并对拍、Tauri、同步服务器 Python、脚本/端到端、Docker Web、Swift、Safari Xcode、Android 单元与 debug APK、三套 Clippy/Rust fmt、三套 `cargo audit`、actionlint 和 72/72 命令矩阵等完整门禁。该记录只说明当时运行结果；当前提交是否可发布必须重新运行本节命令和版本/Markdown 检查，测试数量以最新输出为准。Android 本机验证当时使用 JDK 17、Android SDK Platform 36；由于没有 Android 14+ 真机，不能宣称系统 Credential Manager 交互已实测。

Tauri 审计仍报告 Tauri/GTK 依赖树中的 21 条上游未维护/unsound warning，但没有已知 vulnerability；CI 的 `cargo audit` 为漏洞硬门禁，不使用 `continue-on-error`。Docker 镜像生命周期实测覆盖首次启动、普通重启、测试监护进程下的 `pass-web` 子进程被 `SIGKILL` 后容器 `RestartCount` 增加、强制重建和 `/healthz`。不能用容器内 `kill -9 1` 代替该验证，因为 PID namespace init 的信号语义可能让命令返回成功但主进程不退出。
