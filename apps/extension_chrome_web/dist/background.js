(() => {
  ...
  var diagnosticWriteChain = Promise.resolve();
  var STORAGE_KEY_PASSKEY_DIAGNOSTICS = "pass.webauthn.diagnostics.v1";
  ...
  function safeErrorText(value) {
    return safeString(value).replace(/https?:\/\/\S+/gi, "[url]").replace(/[A-Z0-9._%+-]+@[A-Z0-9.-]+\.[A-Z]{2,}/gi, "[email]").replace(/\b[A-Za-z0-9_-]{32,}\b/g, "[token]").slice(0, 300);
  }
  ...
  function getAuthenticatorDataSummary(credential) {
    ...
    const summary = {
      ...
      backupState: Boolean(flags & 16),
      attestedCredentialData: Boolean(flags & 64),
      extensionDataIncluded: Boolean(flags & 128),
  ...
  function summarizeRequestedExtensions(extensions) {
    ...
    return {
      ...
      googleLegacyAppidSupport: Object.prototype.hasOwnProperty.call(extensions || {}, "googleLegacyAppidSupport") ? Boolean(extensions.googleLegacyAppidSupport) : null
    };
  }
  ...
  function summarizeCreateOptions(publicKey) {
    return {
      ...
      pubKeyCredParams: Array.isArray(publicKey?.pubKeyCredParams) ? publicKey.pubKeyCredParams.map((item) => ({ type: safeString(item?.type), alg: Number(item?.alg) })) : [],
  ...
  function buildCreateSelfChecks({ payload, result, credential, clientData, authenticatorData }) {
    ...
    const algorithms = Array.isArray(request?.pubKeyCredParams) ? request.pubKeyCredParams.map((item) => Number(item?.alg)).filter(Number.isFinite) : [];
    const clientExtensionResultNames = extensionNames(credential?.clientExtensionResults);
    const recognizedOutputs = /* @__PURE__ */ new Set(["appid", "credProps", "hmacCreateSecret", "largeBlob", "prf"]);
    return {
  ...
  function buildPasskeyBridgeDiagnostic({ payload, response, extensionVersion, phase }) {
    ...
    return {
      ...
      request: operation === "create" ? summarizeCreateOptions(publicKey) : {
      ...
      response: {
        ...
        transports: Array.isArray(credential?.response?.transports) ? credential.response.transports.map(safeString) : [],
        clientExtensionResults: {
          names: extensionNames(credential?.clientExtensionResults),
          credPropsRk: credential?.clientExtensionResults?.credProps?.rk === true
        }
      },
      selfChecks: operation === "create" ? buildCreateSelfChecks({ payload, result, credential, clientData, authenticatorData }) : null
    };
  }
  function buildPasskeyPageDiagnostic({ payload, extensionVersion }) {
    ...
    if (phase === "page-credential-returned") {
      diagnostic.pageCredential = {
        ...
        clientExtensionResultNames: Array.isArray(details.clientExtensionResultNames) ? details.clientExtensionResultNames.map(safeString).sort() : [],
        api: {
