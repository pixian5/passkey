...
fn open_db(app: &AppHandle) -> Result<Connection, String> {
    ...
    let db_path = dir.join("pass-copilot-53-tauri.db");
...
fn load_device_name(conn: &Connection) -> Result<String, String> {
    read_kv(conn, KEY_DEVICE_NAME).map(|v| v.unwrap_or_else(|| "Copilot53Desktop".into()))
}
...
fn main() {
    tauri::Builder::default()
        ...
        .expect("error while running copilot-53-tauri");
}
