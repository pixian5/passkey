...
- 在密码框或用户名框聚焦时，内容脚本会向后台查询当前站点可填充账号列表（不含密码），用 closed Shadow DOM 弹出选择面板；选中后再请求密码并填入焦点表单。
- 内容脚本不再缓存全库明文密码；保存/更新提示改为向后台查询 `PASS_CONTENT_CHECK_LOGIN`，并使用常驻右上角浮窗等待用户明确选择。
