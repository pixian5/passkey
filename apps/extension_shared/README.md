...
账号、通行密钥、文件夹和历史记录在 IndexedDB 中使用 AES-GCM 加密；启用主密码后，数据密钥由 PBKDF2-SHA-256（310000 次）派生的密钥包裹，解锁后的数据密钥只保存在浏览器会话存储中，锁定时清除。远端同步使用独立的 256 位同步密钥进行端到端加密，跨平台客户端必须配置同一密钥。
