...
function collectionRecordIdentity(value, collectionKey, index) {
  ...
  const candidates = collectionKey === COLLECTION_ACCOUNTS
    // `accountId` was regenerated by several legacy clients.  It is useful as
    // a final fallback only; `recordId`/`id` are the cross-client identity.
    ? [value.recordId, value.id, value.accountId]
